...
### load data -----------------------------------------------------------------
gerindra_raw <- read_excel_allsheets("dpd-dpc-gerindra-raw.xlsx")
...
for (district in names(gerindra_raw)) {
  ...
  for (j in 1:nrow(district_df)){
    ...
    if (!is.na(district_df[j, ]$`PROV/KAB/KOTA`)) {
      ...
      temp_ketua <- district_df[j, ]$...4 %>% tolower()
      temp_sek <- district_df[j + 1, ]$...4 %>% tolower()
      temp_ben <- district_df[j + 2, ]$...4 %>% tolower()
